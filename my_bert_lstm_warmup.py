"""
                                   和 my_bert_lstm_original.py的区别是加了warm_up
                                   目前这个参数仍旧有一些问题，还要进一步调试
"""
import pandas as pd
import numpy as np
import torch
...
class ModelConfig:
    ...
    save_path = 'bert_bilstm_warmup.pth'  # 模型保存路径


LOG_FILE = 'bert_warmup.log'


def train_model(config, data_train, optimizer, net):
    ...
    for inputs, labels in data_train:
        ...
        loss.backward()
        optimizer.step()  # 如果要用warmup，这一行和下面的一行都必须要用，如果不要warmup，那只要这一行=====================================
        # lr_scheduler.step()
        # wr_schedular.step()  # 添加的一行==============================================================
        ...
        if counter % config.print_every == 0:
            ...
            with open(LOG_FILE, "a") as fout:
                fout.write("Epoch: {}/{}, ".format(e + 1, config.epochs))
                fout.write("Step: {}, ".format(counter))
                fout.write("Train Loss: {:.6f}, ".format(loss.item()))
                fout.write("Val Loss: {:.6f}, ".format(np.mean(val_losses)))
...
def test_model(config, data_test):
    ...
    with open(LOG_FILE, "a") as fout:
        fout.write("Test loss: {:.3f}".format(np.mean(test_losses)) + '\n')
    # accuracy over all test data
    test_acc = num_correct / len(data_test.dataset)
    print("Test accuracy: {:.3f}".format(test_acc))
    with open(LOG_FILE, "a") as fout:
        fout.write("Test accuracy: {:.3f}".format(test_acc) + '\n')
...
if __name__ == '__main__':
    ...
    X_valid, X_test, y_valid, y_test = train_test_split(X_test, y_test, test_size=0.5, shuffle=True, stratify=y_test, random_state=42)
    train_data = TensorDataset(X_train, y_train)
    valid_data = TensorDataset(X_valid, y_valid)
    test_data = TensorDataset(X_test, y_test)
    train_loader = DataLoader(train_data, shuffle=True, batch_size=model_config.batch_size, drop_last=True)
    valid_loader = DataLoader(valid_data, shuffle=True, batch_size=model_config.batch_size, drop_last=True)
    ...
    net = bert_lstm(model_config.bert_path, model_config.hidden_dim, model_config.output_size, model_config.n_layers, model_config.bidirectional)

    optimizer = torch.optim.Adam(net.parameters(), lr=model_config.lr)
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[1, 2, 9], gamma=0.1, )  # 添加的一行======
    wr_schedular = GradualWarmupScheduler(optimizer, multiplier=1.0, total_epoch=1, after_scheduler=lr_scheduler)  # 添加的一行======

    # 记录训练log
    with open(LOG_FILE, "w") as fout:
        fout.write('\n')
        fout.write('==========' * 6)
        fout.write('start trainning: bert')
        fout.write('\n')
    for e in range(model_config.epochs):
        train_model(model_config, train_loader, optimizer, net)
        lr_scheduler.step()
        wr_schedular.step()

    test_model(model_config, test_loader)
    test_comments = ['这个菜真不错，清香扑鼻']
