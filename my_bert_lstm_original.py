...
class ModelConfig:
    ...
    save_path = 'bert_bilstm_original.pth'  # 模型保存路径


LOG_FILE = 'bert_original.log'
...
def train_model(config, data_train):
    ...
    schedular = GradualWarmupScheduler(optimizer, multiplier=10, total_epoch=3, after_scheduler=config.lr)
    if (config.use_cuda):
        net.cuda()
    net.train()
    for e in range(config.epochs):
        ...
        for inputs, labels in data_train:
            ...
            if counter % config.print_every == 0:
                ...
                # print(optimizer.param_groups[0]['lr'])  # 查看学习率 =========================================
